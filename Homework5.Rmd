...
https://github.com/rmharp/stor674-homework.git
...
```bash
# Commands you used
git clone <REPLACE-WITH-YOUR-GITHUB-REPO-URL>
...
git add .
git commit -m "Initial commit: add benchmark script and homework Rmd"
...
WORKDIR /app
# Optionally upgrade pip and install any extras you might need
...
```bash
# Your command here
docker build -t <DOCKERHUB-USERNAME>/stor674-hw5:latest .
```

**Explanation:**

- `docker build`: build a container image from a Dockerfile
- `-t <DOCKERHUB-USERNAME>/stor674-hw5:latest`: tag the image with repo/name and tag for pushing
...
```bash
# Your command here
# CPU-only test (works anywhere)
docker run --rm <DOCKERHUB-USERNAME>/stor674-hw5:latest

# If you have an NVIDIA GPU with Docker + nvidia-container-toolkit installed:
docker run --rm --gpus all <DOCKERHUB-USERNAME>/stor674-hw5:latest
```
...
#
# CPU Computations:
# Time for 10,000,000 elements: 0.9xxx seconds
# Time for 100,000,000 elements: 9.xxxx seconds
#
...
- If GPU wasn’t detected, I added `--gpus all` on `docker run` and ensured the NVIDIA container toolkit was installed.
- For memory errors on very large tensors, I reduced the problem size or increased resources.
- On Apple Silicon, Linux CUDA containers won’t use Apple’s MPS backend; local tests can still run on CPU. For native macOS Python (outside Docker), MPS can be used if enabled, but inside a Linux container it will not apply.
...
git add Dockerfile
git commit -m "Add CUDA-enabled Dockerfile for PyTorch benchmark"
```
...
git switch main
git pull
git merge --no-ff docker-build -m "Merge docker-build into main"
```
...
# Commands you used
docker login
# If you built with the same tag, pushing is:
docker push <DOCKERHUB-USERNAME>/stor674-hw5:latest
# If needed, retag:
# docker tag localname:latest <DOCKERHUB-USERNAME>/stor674-hw5:latest
# docker push <DOCKERHUB-USERNAME>/stor674-hw5:latest
```
...
<DOCKERHUB-USERNAME>/stor674-hw5:latest
```
...
apptainer pull --name stor674-hw5.sif docker://<DOCKERHUB-USERNAME>/stor674-hw5:latest
```
...
apptainer exec --nv stor674-hw5.sif python /app/compute_bench.py
...
IMAGE="$SLURM_SUBMIT_DIR/stor674-hw5.sif"
echo "Running on host: $(hostname)"
echo "Starting at: $(date)"

# Enable NVIDIA support and execute the benchmark inside the container
apptainer exec --nv "$IMAGE" python /app/compute_bench.py
...
<PASTE slurm-<JOBID>.out CONTENTS HERE>
```
...
Containerizing the environment (and versioning both code and Dockerfiles) captures not only algorithms but also dependencies and system libraries, making results portable and reproducible across machines (laptop → cluster). Advantages include easier collaboration, consistent builds, and cleaner deployments to HPC via Apptainer. Challenges include larger images, GPU/runtime driver nuances, and the learning curve for container + scheduler tooling. Good CI/CD and clear documentation mitigate these issues.
